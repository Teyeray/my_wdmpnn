...
def suggest_params(trial):
    return {
        ...
        # === Adapter ===
        "adapter_kind": trial.suggest_categorical(
            "adapter_kind", ["none", "linear", "mlp"]
        ),
...
# ================== Optuna 目标函数 ==================
def make_objective(train_loader, val_loader, df, tasks, max_epochs=100, patience=20):
    def objective(trial):
        ...
        optimizer = torch.optim.Adam(
            [
                ...
                {
                    "params": groups["adapter"],
                    "lr": params["lr_adapter"],
                    "weight_decay": params["weight_decay"],
                },
                {
                    "params": groups["head"],
        ...
        # 分别保存 encoder / adapter / head / full
        ...
        print(
            f"[Trial {trial.number}] Weights saved in {trial_dir} (val_loss={val_loss:.4f})"
        )
        ...
        if trial.number > 0:  # 确保不是第一个 trial
            try:
...
if __name__ == "__main__":
    train_loader, val_loader, df, tasks = load_polymer(
    ...
    study.optimize(
        make_objective(
            train_loader, val_loader, df, tasks,
            max_epochs=2, 
            patience=20
        ),
        n_trials=10,
