...
class OptimizationObjective:
    ...
    def __init__(self, model_name: str, target: str, search_space: dict,
                 base_config: dict, folds: int, seed: int, logger):
    ...
    def __call__(self, trial: optuna.trial.Trial):
        ...
        try:
            # 运行训练
            output = run_training(
                self.model_name, self.target, temp_config_path,
                self.folds, self.seed, self.logger
            )
...
def save_optimization_results(study: optuna.study.Study, model_name: str, 
                            target: str, logger):
    """保存优化结果"""
...
def create_best_config(best_params: dict, base_config: dict, 
                      model_name: str, target: str, logger):
    """创建最佳参数配置文件"""
    ensure_dir("configs")
...
def main():
    ...
    parser.add_argument('--base-config', required=True,
                       help='Base JSON config file path')
    parser.add_argument('--search-space',
                       help='Custom search space JSON file')
    ...
    try:
        ...
        study = optuna.create_study(
            direction='minimize',  # 最小化wMAE
            study_name=study_name
        )
        
        # 创建目标函数
        objective = OptimizationObjective(
            args.model, args.target, search_space, base_config,
            args.folds, args.seed, logger
        )
        ...
        # 标准输出（供外部脚本解析）
        print(f"[TUNE_RESULT] Model={args.model.upper()} | Target={args.target} | "
